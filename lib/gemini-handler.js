...
class GeminiLiveHandler {
    constructor(language, apiKey) {
        ...
        this.ACTIVITY_END_INTERVAL = 3000; // 3초마다 ActivityEnd 신호 (충분한 컨텍스트 확보)
        this.isActivityStarted = false;  // ActivityStart 전송 여부 추적

        // 오디오 수신 추적 (마이크 입력 없이 자동 응답 방지)
        this.lastAudioReceivedTime = 0;
        this.AUDIO_IDLE_THRESHOLD = 2000; // 2초 이상 오디오 없으면 ActivityEnd 전송 안함

        // 오디오 누적량 추적 (너무 짧은 오디오로 응답 방지)
        this.audioBytesSinceLastTurn = 0;
        this.MIN_AUDIO_BYTES_FOR_TURN = 10000; // 최소 10KB (~0.3초) 오디오 필요

        // 🔥 입력 검증 - 유효한 입력이 있을 때만 응답 전달
        this.hasValidInputForTurn = false;
        this.totalInputBytesThisTurn = 0;
        this.MIN_INPUT_BYTES_FOR_RESPONSE = 5000; // 최소 5KB 입력 필요
    }
    ...
    sendAudioChunk(audioChunk) {
        ...
        try {
            ...
            // 오디오 누적량 추적
            this.audioBytesSinceLastTurn += audioChunk.length;

            // 🔥 입력량 추적 - 응답 필터링에 사용
            this.totalInputBytesThisTurn += audioChunk.length;
            if (this.totalInputBytesThisTurn >= this.MIN_INPUT_BYTES_FOR_RESPONSE) {
                this.hasValidInputForTurn = true;
            }
    ...
    handleResponse(message) {
        try {
            ...
            if (message.serverContent?.modelTurn?.parts) {
                message.serverContent.modelTurn.parts.forEach(part => {
                    ...
                    if (part.inlineData && part.inlineData.data) {
                        ...
                        // 🔥 핵심 필터: 충분한 입력 오디오가 있을 때만 응답 전달
                        if (!this.hasValidInputForTurn) {
                            console.log(`🚫 응답 무시 [${this.language}]: 입력 오디오 부족 (${this.totalInputBytesThisTurn || 0} bytes)`);
                            return;
                        }
            ...
            if (message.serverContent?.turnComplete) {
                console.log(`✅ 턴 완료 [${this.language}] (입력: ${this.totalInputBytesThisTurn || 0} bytes)`);
                // 턴 종료 시 입력 추적 초기화
                this.hasValidInputForTurn = false;
                this.totalInputBytesThisTurn = 0;
            }
    ...
    startActivityEndTimer() {
        ...
        this.activityEndTimer = setInterval(() => {
            if (this.session && this.isSessionReady && this.isActivityStarted) {
                ...
                // 최소 오디오 누적량 확인 (너무 짧은 오디오로 응답 방지)
                if (this.audioBytesSinceLastTurn < this.MIN_AUDIO_BYTES_FOR_TURN) {
                    console.log(`⏸️ 오디오 부족 (${this.audioBytesSinceLastTurn}/${this.MIN_AUDIO_BYTES_FOR_TURN} bytes) - ActivityEnd 스킵 [${this.language}]`);
                    return;
                }
                ...
                try {
                    ...
                    console.log(`⏱️ ActivityEnd 신호 전송 [${this.audioBytesSinceLastTurn} bytes] [${this.language}]`);
        ...
        console.log(`⏱️ ActivityEnd 타이머 시작 [${this.language}] (${this.ACTIVITY_END_INTERVAL}ms 간격, 최소 ${this.MIN_AUDIO_BYTES_FOR_TURN} bytes)`);
    }
