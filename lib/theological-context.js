...
class TheologicalContext {
  ...
  buildSystemPrompt(language, context = {}) {
    ...
    if (context.sermonText || (context.keywords && context.keywords.length > 0)) {
      systemMessage += `\n**📋 사전 제공된 참고 자료 (REFERENCE ONLY)**:\n`;
      systemMessage += `⚠️ 아래 내용은 참고용입니다. 실제 화자의 발화 내용과 다를 수 있으니, 항상 실제 음성을 우선하여 번역하세요.\n`;

      if (context.sermonText) {
        systemMessage += `\n오늘의 예상 본문: ${context.sermonText}\n`;
      }

      if (context.keywords && context.keywords.length > 0) {
        systemMessage += `예상 핵심 키워드: ${context.keywords.join(', ')}\n`;
      }
    ...
    // 🚨 중요: 입력 없을 때 침묵 유지
    systemMessage += `\n**🔇 침묵 규칙 (CRITICAL - 절대 위반 금지)**:\n`;
    systemMessage += `- 음성 입력이 없으면 절대 아무 말도 하지 마세요. 완전히 침묵하세요.\n`;
    systemMessage += `- 인사, 소개, 질문, 설명, 확인, 어떤 발화도 하지 마세요.\n`;
    systemMessage += `- 오직 한국어 음성이 들릴 때만 번역을 시작하세요.\n`;
    systemMessage += `- 번역할 음성이 없으면 아무런 소리도 내지 마세요.\n`;

    // 🚨 중요: 반복 번역 방지
    systemMessage += `\n**🔄 반복 금지 규칙 (CRITICAL - 절대 위반 금지)**:\n`;
    systemMessage += `- 이미 번역한 내용을 절대 다시 말하지 마세요.\n`;
    systemMessage += `- 같은 문장이나 비슷한 내용을 반복하지 마세요.\n`;
    systemMessage += `- 오직 새로 들리는 음성만 번역하세요.\n`;
    systemMessage += `- 이전에 번역한 것을 기억하고 건너뛰세요.\n`;
    systemMessage += `- 화자가 반복하지 않는 한 당신도 반복하지 마세요.\n`;
